...
public static class ScriptService
{
    ...
    public static async Task<string> RunAsync(ScriptEntry script)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                luaScript.Globals["_fracture_context"] = context;

                // Register C# callbacks
                luaScript.Globals["_fracture_log"] = (Action<string>)(msg =>
                {
                    string ts = DateTime.Now.ToString("HH:mm:ss");
                    output.AppendLine($"[{ts}] {msg}");
                });

                luaScript.Globals["_fracture_notify"] = (Action<string, string, string>)((title, message, type) =>
                {
                ...
                luaScript.Globals["_fracture_set_clipboard"] = (Action<string>)(text =>
                {
                    clipboard = text;
                });

                luaScript.Globals["_fracture_launch_game"] = (Action<string, long>)((accountName, placeId) =>
                {
                ...
                luaScript.Globals["_fracture_sleep"] = (Action<int>)(ms =>
                {
                    Thread.Sleep(ms);
                });
                ...
                luaScript.Globals["_fracture_ui_create_tab"] = (Func<string, string, int>)((name, icon) =>
                {
                ...
                luaScript.Globals["_fracture_ui_add_label"] = (Action<int, string, double, bool>)((handle, text, fontSize, bold) =>
                {
                ...
                luaScript.Globals["_fracture_ui_add_button"] = (Action<int, string, Closure>)((handle, text, callback) =>
                {
                    if (uiTabs.TryGetValue(handle, out var tab))
                    {
                ...
                luaScript.Globals["_fracture_ui_add_text_input"] = (Action<int, string, string>)((handle, id, placeholder) =>
                {
                    if (uiTabs.TryGetValue(handle, out var tab))
                        ScriptUIService.AddElement(tab, new ScriptUIElement
                        {
                ...
                luaScript.Globals["_fracture_ui_add_separator"] = (Action<int>)((handle) =>
                {
                    if (uiTabs.TryGetValue(handle, out var tab))
                        ScriptUIService.AddElement(tab, new ScriptUIElement { Type = UIElementType.Separator });
                });

                luaScript.Globals["_fracture_ui_add_progress"] = (Action<int, double, string>)((handle, value, label) =>
                {
                    if (uiTabs.TryGetValue(handle, out var tab))
                        ScriptUIService.AddElement(tab, new ScriptUIElement
                        {
                ...
                luaScript.Globals["_fracture_ui_add_checkbox"] = (Action<int, string, string, bool>)((handle, id, text, isChecked) =>
                {
                    if (uiTabs.TryGetValue(handle, out var tab))
                        ScriptUIService.AddElement(tab, new ScriptUIElement
                        {
                ...
                luaScript.Globals["_fracture_ui_add_dropdown"] = (Action<int, string, string, Table, int>)((handle, id, label, options, selectedIdx) =>
                {
                    if (uiTabs.TryGetValue(handle, out var tab))
                    {
                ...
                luaScript.Globals["_fracture_ui_get_value"] = (Func<int, string, string>)((handle, id) =>
                {
                    if (uiTabs.TryGetValue(handle, out var tab))
                ...
                luaScript.DoString(apiLua, null, "fracture_api");
    ...
    private static Table BuildContextTable(Script luaScript)
    {
        ...
        ctx["DataDir"] = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Fracture");
    ...
    private static string LoadEmbeddedLuaApi()
    {
        using var stream = Assembly.GetExecutingAssembly()
            .GetManifestResourceStream("Fracture.Resources.fracture_api.lua");
