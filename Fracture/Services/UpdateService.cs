...
public static class UpdateService
{
    ...
    public const string GitHubRepo = "coder-isxt/fracture";

    private static readonly HttpClient _http = new()
    {
        DefaultRequestHeaders = { { "User-Agent", "Fracture-Updater" } }
    };
    ...
    public static async Task<string?> DownloadUpdateAsync(GitHubRelease release, IProgress<double>? progress = null)
    {
        ...
        try
        {
            string tempPath = Path.Combine(Path.GetTempPath(), "Fracture_Update.exe");
    ...
    public static void ApplyUpdate(string downloadedExePath)
    {
        ...
        string batchPath = Path.Combine(Path.GetTempPath(), "fracture_update.bat");
        string batch = $"""
            @echo off
