...
{
    private static readonly byte[] ExportKey = Encoding.UTF8.GetBytes("Fracture_Export_Key_32B!@#$%^&*()12");
    private static readonly byte[] ExportIV = Encoding.UTF8.GetBytes("Fracture_IV_16B!@#");
    ...
    public static void Export(List<RobloxAccount> accounts, string filePath)
    {
        ...
        // Write magic header
        byte[] magic = "FRACTURE"u8.ToArray();
    ...
    public static List<RobloxAccount> Import(string filePath)
    {
        ...
        if (header != "FRACTURE")
            throw new InvalidDataException("Not a valid Fracture export file.");
